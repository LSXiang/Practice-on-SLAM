...
public:
    ...
    {
        Sophus::SE3 up(
            Sophus::SO3(update[0], update[1], update[2]),
...
/* edge of lie algebra */
class EdgeProject3dPoseOnly : public g2o::BaseUnaryEdge<3, Eigen::Vector3d, VertexSE3LieAlgebra>
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
    
    EdgeProject3dPoseOnly(const Eigen::Vector3d& point) : _point(point) {}
    ...
    virtual void computeError()
    {
        ...
        _error = _measurement - pose * _point;
    }
    
    virtual void linearizeOplus()
    {
        Sophus::SE3 pose = static_cast<VertexSE3LieAlgebra*>(_vertices[0])->estimate();
        Eigen::Vector3d xyz_trans = pose * _point;
...
int main(int argc, char** argv)
{
    ...
    if (img_1.empty() || img_2.empty()) {
        std::cerr << "img1 or img2 is empty." << std::endl;
        return 1;
    }
    ...
    if (depth1.empty() || depth2.empty()) {
        std::cerr << "depth1 or depth2 is empty." << std::endl;
        return 1;
    }
    ...
    std::cout << "ICP vid SVD results: " << std::endl;
...
void bundleAdjustment(const std::vector<cv::Point3f>& pts1, const std::vector<cv::Point3f>& pts2, cv::Mat& R, cv::Mat& t)
{
    ...
    // vectex
    VertexSE3LieAlgebra* pose = new VertexSE3LieAlgebra();
    pose->setId(0);
    pose->setEstimate(Sophus::SE3(Eigen::Matrix3d::Identity(), Eigen::Vector3d(0, 0, 0)));
    optimizer.addVertex(pose);
    
    // edge
    int index = 1;
    std::vector<EdgeProject3dPoseOnly*> edges;
    for (size_t i = 0; i < pts1.size(); i ++) {
        EdgeProject3dPoseOnly* edge = new EdgeProject3dPoseOnly(Eigen::Vector3d(pts2[i].x, pts2[i].y, pts2[i].z));
        edge->setId(index);
        edge->setVertex(0, dynamic_cast<VertexSE3LieAlgebra*>(pose));
    ...
    std::cout << "optimization costs time: " << time_used.count() << " seconds." << "\r\n";
